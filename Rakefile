...
#
  ...
  task :gemfile do
      ...
      if test(?e, 'Gemfile.erb')
        ...
        Template {
          <<-__
            ...
            gem(*<%= Array(dependency).flatten.inspect %>)
            <% end %>
          __
        }
      end
    ...
    gemfile = "Gemfile"
    open(gemfile, "w"){|fd| fd.puts(template)}
  ...
  task :gem => [:clean, :gemspec, :gemfile] do
    Fu.mkdir_p(This.pkgdir)
    before = Dir['*.gem']
...
#
  ...
  This._lib    = "#{ This.dir }/lib/_#{ This.lib }.rb" 
...
#
  ...
  if added.size > 1
    STDERR.puts "WARNING: defined multiple constants #{ added.inspect } in #{ This._lib }, using #{ This.const } !!!"
  end
